...
impl StringHandler for CommandHandler {
    ...
    fn set(&mut self, key: &str, value: StringValue, opts: Option<SetOptions>) -> bool {
        self.mem_storage.set(key, value, opts)
    }
...
mod tests {
    ...
    fn test_string_commands() {
        ...
        assert!(handler.set("key1", StringValue::Raw("value1".to_string()), None));
        assert_eq!(
            handler.get("key1"),
        ...
        assert_eq!(
            ...
            vec![
                Some(StringValue::Raw("value1".to_string())),
                Some(StringValue::Int(3))
            ]
        );
        assert!(handler.mset(vec![
            ("key2".to_string(), StringValue::Raw("value2".to_string())),
            ("key3".to_string(), StringValue::Raw("value3".to_string()))
        ]));
